...
export default function HomePage() {
  ...
  const zoomAt = (clientX: number, clientY: number, nextScale: number) => { const svg = svgRef.current; const matrix = svg?.getScreenCTM()?.inverse(); if (!svg || !matrix) return; const point = svg.createSVGPoint(); point.x = clientX; point.y = clientY; const viewPoint = point.matrixTransform(matrix); const acceleratedScale = Math.max(.4, Math.min(12, scale + (nextScale - scale) * 2)); setOffset(current => ({ x: viewPoint.x - (viewPoint.x - current.x) * acceleratedScale / scale, y: viewPoint.y - (viewPoint.y - current.y) * acceleratedScale / scale })); setScale(acceleratedScale); };
  ...
  const handlePointerMove = (event: React.PointerEvent<SVGSVGElement>) => { if (!pointers.current.has(event.pointerId)) return; pointers.current.set(event.pointerId, { x: event.clientX, y: event.clientY }); const points = [...pointers.current.values()]; if (points.length >= 2 && pinchStart.current) { const distance = Math.hypot(points[0].x - points[1].x, points[0].y - points[1].y); const nextScale = Math.max(.4, Math.min(12, pinchStart.current.scale * distance / pinchStart.current.distance)); const midpoint = svgPoint((points[0].x + points[1].x) / 2, (points[0].y + points[1].y) / 2); const worldPoint = { x: (midpoint.x - offset.x) / scale, y: (midpoint.y - offset.y) / scale }; setScale(nextScale); setOffset({ x: midpoint.x - worldPoint.x * nextScale, y: midpoint.y - worldPoint.y * nextScale }); return; } if (points.length === 1 && panStart.current) { const rect = svgRef.current!.getBoundingClientRect(); const movementMultiplier = 2; const dx = (event.clientX - panStart.current.x) * layout.width / rect.width * movementMultiplier; const dy = (event.clientY - panStart.current.y) * layout.height / rect.height * movementMultiplier; if (Math.abs(dx) > 4 || Math.abs(dy) > 4) didDrag.current = true; setOffset({ x: panStart.current.offset.x + dx, y: panStart.current.offset.y + dy }); } };
